 ...
 * u, v, e, f, q, g are the relative scores
 ...
 *  v(x, y) = h(x, y) - u(x, y -1) // v is only used in backtracing
 *
...
typedef void (*banded_striped_epi8_seqalign_piecex_row_init_func)(b1i *us[2], b1i *es[2], b1i *qs[2], b1i *vs, int *begs, int mode, u4i bandwidth, b1i max_nt_score, b1i min_nt_score, b1i gapo1, b1i gape1, b1i gapo2, b1i gape2);
...
typedef int (*banded_striped_epi8_seqalign_piecex_row_cal_func)(u4i rbeg, u1i base, b1i *us[2], b1i *es[2], b1i *qs[2], b1i *vs, b1i *qprof, b1i gapo1, b1i gape1, b1i gapo2, b1i gape2, u4i W, u4i mov, int ph, int rh);
...
static void banded_striped_epi8_seqalign_piecex_row_init(b1i *us[2], b1i *es[2], b1i *qs[2], b1i *vs, int *begs, int mode, u4i bandwidth, b1i max_nt, b1i min_nt, b1i gapo1, b1i gape1, b1i gapo2, b1i gape2){
	xint ZERO, MIN, GAP;
	u4i k, W, xp;
	...
	if(mode == SEQALIGN_MODE_GLOBAL || mode == SEQALIGN_MODE_EXTEND){
		...
		for(k=0;k<W;k++) mm_store(((xint*)vs) + k, ZERO);
		vs[0] = 1;
		begs[-1] = - Int(bandwidth);
	} else {
		for(k=0;k<W;k++) mm_store(((xint*)vs) + k, MIN);
...
static inline int banded_striped_epi8_seqalign_piece0_row_cal(u4i rbeg, u1i base, b1i *us[2], b1i *es[2], b1i *qs[2], b1i *vs, b1i *qprof, b1i gapo1, b1i gape1, b1i gapo2, b1i gape2, u4i W, u4i mov, int ph, int rh){
	...
	if(mov){
		i = W * WORDSIZE - mov;
		if(us[1][banded_striped_epi8_pos2idx(W << WORDSHIFT, i)] + vs[banded_striped_epi8_pos2idx(W << WORDSHIFT, i - 1)] == banded_striped_epi8_seqalign_get_qprof_value(qprof, rbeg + i, base)){
			vs[banded_striped_epi8_pos2idx(W << WORDSHIFT, i)] = 1;
		} else {
			vs[banded_striped_epi8_pos2idx(W << WORDSHIFT, i)] = 0;
		}
		for(i++;i<W * WORDSIZE;i++){
			vs[banded_striped_epi8_pos2idx(W << WORDSHIFT, i)] = 0;
		}
	}
...
static inline void banded_striped_epi8_seqalign_row_print(FILE *out, u1i *qseq, u4i tidx, u4i tpos, u1i tbase, u4i bandwidth, u4i mov, u4i rbeg, u4i rmax, int max_score, int shift, b1i *us, b1i *vs, int wl, int wr, int detail){
	u4i i, x, W;
	int score;
	W = bandwidth / WORDSIZE;
	...
	if(detail){
		...
		for(i=0;i<bandwidth;i++){
			...
			fprintf(out, "\t%d:%c%d:%d:%d", i + rbeg, "ACGTN-"[qseq[rbeg + i]], score + us[x], us[x], vs[x]);
			score += us[x];
		}
	}
	fprintf(out, "\n");
...
static inline u4i banded_striped_epi8_seqalign_piecex_trace(u1i *qseq, u1i *tseq, b1i **ups, b1i **vps, int *begs, u4i bandwidth, b1i *matrix, b1i gapo1, b1i gape1, b1i gapo2, b1i gape2, seqalign_result_t *rs, u4v *cigars){
	...
	while(1){
#define fetch_ups(x, y) ups[(y) + 1][banded_striped_epi8_pos2idx(W << WORDSHIFT, (x) - begs[(y)])]
#define fetch_vps(x, y) vps[(y) + 1][banded_striped_epi8_pos2idx(W << WORDSHIFT, (x) - begs[(y)])]
...
static inline seqalign_result_t banded_striped_epi8_seqalign_pairwise(u1i *qseq, u4i qlen, u1i *tseq, u4i tlen, b1v *mempool, u4v *cigars, int mode, u4i bandwidth, b1i matrix[16], b1i gapo1, b1i gape1, b1i gapo2, b1i gape2, int verbose){
	...
	banded_striped_epi8_seqalign_piecex_trace_func  backtrace;
	seqalign_result_t rs;
	b1i *memp, *mempb, *qprof, *us[2], *es[2], *qs[2], *vs, **ups, **vps;
	u8i mpsize;
	...
	backtrace  = banded_striped_epi8_seqalign_piecex_trace;
	if(gapo2 < gapo1 && gape2 > gape1 && gapo2 + gape2 < gapo1 + gape1){
		piecewise = 2;
	...
	mpsize += roundup_times((tlen + 1) * sizeof(void*), WORDSIZE) * 2; // ups[] and vps[]
	mpsize += bandwidth * ((tlen + 1) * 2 + 1 + piecewise * 2); // ups[][], vps[][], us[0][], es[0/1][], qs[0/1][]
	...
	ups = (b1i**)memp; memp += roundup_times((tlen + 1) * sizeof(void*), WORDSIZE);
	vps = (b1i**)memp; memp += roundup_times((tlen + 1) * sizeof(void*), WORDSIZE);
	for(i=0;i<=tlen;i++){ ups[i] = (b1i*)memp; memp += roundup_times(W * sizeof(void*), WORDSIZE); }
	us[1] = ups[0];
	for(i=0;i<=tlen;i++){ vps[i] = (b1i*)memp; memp += roundup_times(W * sizeof(void*), WORDSIZE); }
	vs = vps[0];
	for(k=0;k<=tlen;k++){ ups[k] = (b1i*)memp; memp += bandwidth; }
	for(k=0;k<=tlen;k++){ vps[k] = (b1i*)memp; memp += bandwidth; }
	...
	row_init(us, es, qs, vs, begs, mode, bandwidth, smax, smin, gapo1, gape1, gapo2, gape2);
	// loop rows
	rbeg = rmax = 0;
	mov = 0;
	hh = 0;
	...
	for(i=0;i<tlen;i++){
		...
		us[1] = ups[i + 1];
		vs = vps[i + 1];
		hh = row_cal(rbeg, tbase, us, es, qs, vs, qprof, gapo1, gape1, gapo2, gape2, W, mov, ph, rh);
		if(verbose > 2){
		...
		wt = W * smax;
		if(verbose){
			banded_striped_epi8_seqalign_row_print(stdout, qseq, 1, i, tbase, bandwidth, mov, rbeg, rmax, lst_score, hh, us[1], vs, wl, wr, verbose > 1);
		}
		if(i < (WORDSIZE / 2) * W){
			rbx = 0;
		} else if(wl + wt < wr){
	...
	// backtrace
	backtrace(qseq, tseq, ups, vps, begs, bandwidth, matrix, gapo1, gape1, gapo2, gape2, &rs, cigars);
	if(mempb) free(mempb);
	return rs;
}
